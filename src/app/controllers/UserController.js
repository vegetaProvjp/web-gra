...
class UserController {
  ...
  async cancelled(req, res, next) {
    ...
    const order1 = await Order.updateOne({ _id: idOrder }, {
      $set: {
        status: 'cancelled'
      }
    })
  ...
  checkout_address(req, res, next) {
    res.render('user/checkout-address')
  }

  async post_checkout_address(req, res, next) {
    ...
    fee = await getFee(ward, district, province)
    fee = fee.data.total
  ...
  success(req, res, next) {
    ...
    paypal.payment.execute(paymentId, execute_payment_json, async function (error, payment) {
      if (error) {
        ...
        console.log('--------.')
        console.log(req.session.dataOrder.cart)
        var obj = req.session.dataOrder.cart.items
        for (var id in obj) {
          await Product.updateOne({ _id: id }, {
            $inc: {
              "quantity": -obj[id]['qty']
        ...
        console.log(req.session.dataOrder.address)
        await Order.create({
          user: userID,
          cart: req.session.dataOrder.cart,
          address: req.session.dataOrder.address,
          name: req.session.dataOrder.name,
          message: req.session.dataOrder.message,
          pay: req.session.dataOrder.pay,
        })
  ...
  async post_checkout(req, res, next) {
    ...
    for (var id in obj) {
      ...
      if (product.quantity <= obj[id]['qty']) {
        return res.json(`Mặt hàng ${product.name} đã hết`)
      }
    }
    if (pay == 1) {
      var dataOrder = {
      ...
      req.session.dataOrder = dataOrder
      console.log(req.session.dataOrder)
      for (var id in obj) {
        items.push({
